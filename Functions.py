...
def common_dependencies(real_data, synthetic_data, features):
    ...
    for feature1 in features:
        for feature2 in features:
            if feature1 != feature2:  # Avoid redundant pairs
                ...
                # Check if the real conditional probabilities contain values other than 0 and 1 (if the values are combinations of 0 and 1                     then they are functionally dependent and we do not want to consider that perticular condition into logical dependency)
                if not ((conditional_probabilities_real == 0) | (conditional_probabilities_real == 1)).all().all():
    ...
    print('Total number of logical dependencies in synthetic data are:', match)
    print('The percentage of dependencies preserved in synthetic data is:', (match / count) * 100)
